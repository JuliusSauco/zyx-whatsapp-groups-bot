...
let handler = async (m, { text, usedPrefix, command, conn }) => {
  ...
  let img =
    "https://cinefilosoficial.com/wp-content/uploads/2021/07/cuevana.jpg";
  if (command == "pelisplus")
    img =
  ...
  let res = await aaaa.map(
    (v) => `*🎬 • Nombre:* ${v.title}\n*🍿 • Url:* ${v.link}`
  ).join`\n\n───────────────\n\n`;
  let ads =
    "*💫 • Bloqueador de anuncios recomendado:* Block This\n*⛨ • Link:* https://block-this.com/block-this-latest.apk\n\n≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣\n\n";
  conn.sendMessage(
    m.chat,
    { image: { url: img }, caption: ads + res },
    { quoted: m }
  );
};
