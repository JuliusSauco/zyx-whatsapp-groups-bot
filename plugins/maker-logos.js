...
let handler = async (
  m,
  { conn, args: [effect], text: txt, usedPrefix, command, name }
) => {
  if (!effect)
    throw (
  ...
  if (!effects.find((v) => new RegExp(v.title, "gi").test(effect)))
    throw `*[❗𝐈𝐍𝐅𝐎❗] 𝙴𝙻 𝙴𝙵𝙴𝙲𝚃𝙾 ${effect} 𝙽𝙾 𝙴𝚂𝚃𝙰 𝙴𝙽 𝙻𝙰 𝙻𝙸𝚂𝚃𝙰 𝙳𝙴 𝙴𝙵𝙴𝙲𝚃𝙾𝚂*`;
...
async function textpro(effect, ...texts) {
  ...
  let cookieParse = (cookie, query) =>
    cookie.includes(query + "=")
      ? cookie.split(query + "=")[1].split(";")[0]
      : "undefined";
  ...
  let body = Object.keys(token2)
    ...
    .join("&");
  let resImgUrl = await fetch(
    `https://textpro.me/effect/create-image?${body}`,
    {
      headers: {
        Accept: "*/*",
        "Accept-Language": "en-US,en;q=0.9",
