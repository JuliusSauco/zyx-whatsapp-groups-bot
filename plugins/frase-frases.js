...
let handler = async (m, { conn, command }) => {
  if (command == "consejo") {
    ...
    let res = await fetch(
      `https://latam-api.vercel.app/api/rand_aviso?apikey=brunosobrino`
    );
  ...
  if (command == "fraseromantica") {
    let res = await fetch(
      "https://api.zahwazein.xyz/randomtext/bucinquote?apikey=hdiiofficial"
    );
    let json = await res.json();
    let frase = json.result.message;
    let frase1 = await translate(frase, { to: "es", autoCorrect: true }).catch(
      (_) => null
    );
    m.reply(`*╭─◆────◈⚘◈─────◆─╮*\n*❥  ${frase1.text}*\n*╰─◆────◈⚘◈─────◆─╯*`);
  }
  ...
  if (command == "historiaromantica") {
    try {
      ...
      let storytime = await translate(cerpe.cerita, {
        to: "es",
        autoCorrect: true,
      }).catch((_) => null);
      let titletime = await translate(cerpe.title, {
        to: "es",
        autoCorrect: true,
      }).catch((_) => null);
      conn.reply(
