...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  if (!/image\/(jpe?g|png)/.test(mime))
    throw `*[❗] 𝙴𝙻 𝚃𝙸𝙿𝙾 𝙳𝙴 𝙰𝚁𝙲𝙷𝙸𝚅𝙾 ${mime} 𝙽𝙾 𝙴𝚂 𝙲𝙾𝚁𝚁𝙴𝙲𝚃𝙾, 𝚁𝙴𝙲𝚄𝙴𝚁𝙳𝙴 𝚀𝚄𝙴 𝙳𝙴𝙱𝙴 𝚂𝙴𝚁 𝙸𝙼𝙰𝙶𝙴𝙽, 𝙹𝙿𝙶, 𝙹𝙿𝙴𝙶 𝙾 𝙿𝙽𝙶*`;
  if (!text)
...
—◉ ${
      usedPrefix + command
    } artist_in_the_dark <responder / etiquetar a una imagen>
...
° ඬ⃟💫 ${usedPrefix + command} yellow_wall`);
  ...
  let images = `https://violetics.pw/api/photomaker/${encodeURIComponent(
    text
  )}?apikey=beta&image=${encodeURIComponent(url)}`;
  let caption = `*⎔┉━「 𝐏𝐇𝐌𝐀𝐊𝐄𝐑 」━┉⎔*
*💟 𝙴𝙵𝙴𝙲𝚃𝙾:* ${text}`;
  conn.sendButton(
    m.chat,
    ...
    wm,
    images,
    [["💫 𝙼𝙰𝚂 𝙾𝙿𝙲𝙸𝙾𝙽𝙴𝚂 💫", `${usedPrefix}phmakerlist`]],
    m
  );
};
