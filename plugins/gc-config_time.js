...
`;
  ...
  await conn.groupSettingUpdate(m.chat, isClose).then(async (_) => {
    m.reply(
      `⚠️ *_Grupo ${isClose == "announcement" ? "cerrado" : "abierto"} ${
        args[1] ? `durante *${clockString(timeoutset)}_*` : ""
      }`
    );
  });
  if (args[1]) {
          ...
          conn.reply(
            ...
            `${
              isClose == "not_announcement"
                ...
                : "*El grupo se ha abierto, ¡ahora todos los miembros pueden enviar mensajes!*"
            }!`
          );
        });
    }, timeoutset);
