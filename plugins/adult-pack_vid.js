...
let handler = async (m, { conn, command, usedPrefix }) => {
  ...
  switch (command) {
    ...
    case "videoxxx":
    case "vídeoxxx":
      let url4 = await videosxxxc[
        Math.floor(Math.random() * videosxxxc.length)
      ];
    ...
    case "pornolesbianav":
    case "pornolesv":
      let url5 = await videosxxxc2[
        Math.floor(Math.random() * videosxxxc2.length)
      ];
      await conn.sendButton(
        m.chat,
        `*ᴅɪsғʀᴜᴛᴀ ᴅᴇʟ ᴠɪᴅᴇᴏ 🥵*`,
...
handler.command =
  /^(pack|pack2|pack3|videoxxx|vídeoxxx|videoxxxlesbi|videolesbixxx|pornolesbivid|pornolesbianavid|pornolesbiv|pornolesbianav|pornolesv)$/i;
