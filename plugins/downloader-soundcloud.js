...
let handler = async (m, { conn, text }) => {
  ...
  try {
    let res = await fetch(
      `https://api.akuari.my.id/search/searchsoundcloud?query=${text}`
    );
    let json2 = await res.json();
    let urlSC = await json2.hasil[0].url;
    let res2 = await fetch(
      `https://api.akuari.my.id/downloader/scdl?link=${urlSC}`
    );
    let json = await res2.json();
    let shortUrl = await (
      await fetch(`https://tinyurl.com/api-create.php?url=${json.link}`)
    ).text();
