...
let handler = async (m, { command, conn }) => {
  ...
  if (command == "nsfwass") {
    let res = (
      await axios.get(
        `https://raw.githubusercontent.com/BrunoSobrino/TheMystic-Bot-MD/master/src/JSON/nsfwass.json`
      )
    ).data;
    let haha = await res[Math.floor(res.length * Math.random())];
    conn.sendButton(
  ...
  if (command == "tetas") {
    ...
    let res = await conn.getFile(
      `https://api-fgmods.ddns.net/api/nsfw/boobs?apikey=fg-dylux`
    ).data;
    if (res == "" || !res || res == null)
  ...
  if (command == "booty") {
    ...
    let res = await conn.getFile(
      `https://api-fgmods.ddns.net/api/nsfw/ass?apikey=fg-dylux`
    ).data;
    if (res == "" || !res || res == null)
      res = await resError[Math.floor(resError.length * Math.random())];
    conn.sendButton(
  ...
  if (command == "imagenlesbians") {
    ...
    let res = await conn.getFile(
      `https://api-fgmods.ddns.net/api/nsfw/lesbian?apikey=fg-dylux`
    ).data;
    if (res == "" || !res || res == null)
      res = await resError[Math.floor(resError.length * Math.random())];
    conn.sendButton(
  ...
  if (command == "pechos") {
    let res = (
      await axios.get(
        `https://raw.githubusercontent.com/BrunoSobrino/TheMystic-Bot-MD/master/src/JSON/pechos.json`
      )
    ).data;
    let url = await res[Math.floor(res.length * Math.random())];
    conn.sendButton(
