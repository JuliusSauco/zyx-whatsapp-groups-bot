...
let handler = async (m, { command, conn }) => {
  ...
  if (command == "wprandom") {
    let res = (
      await axios.get(
        `https://raw.githubusercontent.com/BrunoSobrino/TheMystic-Bot-MD/master/src/JSON/wprandom.json`
      )
    ).data;
  ...
  if (command == "randomprofile") {
    let haha = await conn.getFile(
      `https://api.zahwazein.xyz/randomimage/profil?apikey=${apikey}`
    );
    await conn.reply(m.chat, global.wait, m);
    conn.sendButton(
      m.chat,
  ...
  if (command == "wpaesthetic2") {
    let haha = await conn.getFile(
      `https://api.zahwazein.xyz/randomimage/aesthetic?apikey=${apikey}`
    );
    await conn.reply(m.chat, global.wait, m);
    conn.sendButton(
      m.chat,
  ...
  if (command == "wpvehiculo") {
    let haha = await conn.getFile(
      `https://api.zahwazein.xyz/randomimage/mobil?apikey=${apikey}`
    );
    await conn.reply(m.chat, global.wait, m);
    conn.sendButton(
      m.chat,
  ...
  if (command == "wallhp") {
    let haha = await conn.getFile(
      `https://api.zahwazein.xyz/randomimage/wallhp?apikey=${apikey}`
    );
    await conn.reply(m.chat, global.wait, m);
    conn.sendButton(
      m.chat,
  ...
  if (command == "wpmoto") {
    let haha = await conn.getFile(
      `https://api.zahwazein.xyz/randomimage/motor?apikey=${apikey}`
    );
    await conn.reply(m.chat, global.wait, m);
    conn.sendButton(
      m.chat,
